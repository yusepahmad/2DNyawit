...
public class Player extends Entity {
    ...
    private void setDefaultValues() {
        setWorldX(this.tileSize * 42); // Branch B start position
        setWorldY(this.tileSize * 4);
        setSpeed(3);
        setDirection("down");

        // Player Status (Branch A Features)
        maxLife = 6;
        life = maxLife;
        gold = 1000; // Branch B starting gold
        strength = 1;
    ...
    private void getPlayerImage() {
        mainCharacter = setup("/player/walking/rat", this.tileSize, this.tileSize);
        up1 = setup("/player/walking/rat-up", this.tileSize, this.tileSize);
        up2 = setup("/player/walking/rat-up", this.tileSize, this.tileSize);
        down1 = setup("/player/walking/rat-down", this.tileSize, this.tileSize);
        down2 = setup("/player/walking/rat-down", this.tileSize, this.tileSize);
        left1 = setup("/player/walking/rat-run-to-left", this.tileSize, this.tileSize);
        left2 = setup("/player/walking/rat-run-to-left", this.tileSize, this.tileSize);
        right1 = setup("/player/walking/rat-run-to-right", this.tileSize, this.tileSize);
        right2 = setup("/player/walking/rat-run-to-right", this.tileSize, this.tileSize);
    }

    private void getPlayerAttackImage() {
        if(currentWeapon.getType() == Type.AXE) {
            this.attackDown1 = setup("/player/attacks/boy_axe_down_1", this.gp.getTileSize(), this.gp.getTileSize()*2);
            this.attackDown2 = setup("/player/attacks/boy_axe_down_2", this.gp.getTileSize(), this.gp.getTileSize()*2);
            this.attackUp1 = setup("/player/attacks/boy_axe_up_1", this.gp.getTileSize(), this.gp.getTileSize()*2);
            this.attackUp2 = setup("/player/attacks/boy_axe_up_2", this.gp.getTileSize(), this.gp.getTileSize()*2);
            this.attackRight1 = setup("/player/attacks/boy_axe_right_1", this.gp.getTileSize()*2, this.gp.getTileSize());
            this.attackRight2 = setup("/player/attacks/boy_axe_right_2", this.gp.getTileSize()*2, this.gp.getTileSize());
            this.attackLeft1 = setup("/player/attacks/boy_axe_left_1", this.gp.getTileSize()*2, this.gp.getTileSize());
            this.attackLeft2 = setup("/player/attacks/boy_axe_left_2", this.gp.getTileSize()*2, this.gp.getTileSize());
        }
    }
    ...
    public void attacking() {
        ...
        if(this.spriteCounter > 5 && this.spriteCounter <= 25) {
            ...
            switch (getDirection()) {
                case "up":
                    this.worldY -= this.attackArea.height; break;
                case "down":
                    this.worldY += this.attackArea.height; break;
                case "left":
                    this.worldX -= this.attackArea.width; break;
                case "right":
                    this.worldX += this.attackArea.width; break;
            }
    ...
    public void damageInteractiveTile(int i) {
        if(i != 999 && this.gp.getInteractiveTileIsAttackable(i, this) && this.gp.getInteractiveTileInvincible(i) == false) {
